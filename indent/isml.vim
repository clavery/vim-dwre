...
fun! HtmlIndentGet(lnum)
    ...
    if   0 < searchpair(isml, '', ismle, 'nWb')
    \ && 0 < searchpair(isml, '', ismle, 'nW')
        " we're inside javascript
        if getline(lnum) !~ isml && getline(a:lnum) !~ ismle
            if restore_ic == 0
              setlocal noic
            endif
            if s:jsindent == ''
        ...
        if getline(a:lnum) =~ ismle
          return indent(searchpair(isml, '', ismle, 'nWb'))
        endif
    endif
